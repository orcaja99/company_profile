...
@section('admin-content')
    <h1>Edit Data Cara Daftar Sabungan Baru</h1>

   <form action="{{ route('admin.dsb.update', $dsb->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="form-group">
            <label for="judul">Judul</label>
            <textarea class="form-control tiny-mce" name="judul" id="judul" rows="5" style="height: 200px;">{{ $dsb->judul }}</textarea>
            @error('judul')
        ...
        <div class="form-group">
            ...
            <textarea class="form-control tiny-mce" name="keterangan" id="keterangan" rows="5" style="height: 200px;">{{ $dsb->keterangan }}</textarea>
            @error('keterangan')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
